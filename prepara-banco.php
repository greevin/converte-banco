...
foreach($tables as $table) {
    $sql = 'TRUNCATE TABLE '.$migrate_to_db.'.'.$table['TABLE_NAME'];
...
INSERT INTO '.$migrate_to_db.".wp_db_terms (name, slug, term_group)
...
REPLACE INTO '.$migrate_to_db.'.wp_db_terms (name, slug)
SELECT n.name, n.type
FROM '.$migrate_from_db.".node_type n;
";
...
$add_new_taxonomies = '
REPLACE INTO '.$migrate_to_db.'.wp_db_term_taxonomy (term_taxonomy_id, term_id, taxonomy, description, parent)
SELECT t.term_id, t.term_id "term_id", "category", "", 0
FROM '.$migrate_to_db.'.wp_db_terms t;
';
...
INSERT INTO '.$migrate_to_db.'.wp_db_term_relationships (object_id, term_taxonomy_id)
SELECT node.nid, '.$migrate_to_db.'.wp_db_terms.term_id
FROM '.$migrate_from_db.'.node
INNER JOIN '.$migrate_to_db.'.wp_db_terms ON '.$migrate_to_db.'.wp_db_terms.slug = node.type;
';
...
$update_tags_counter = '
UPDATE '.$migrate_to_db.'.wp_db_term_taxonomy tt
SET count = (SELECT COUNT(tr.object_id)
FROM '.$migrate_to_db.'.wp_db_term_relationships tr
